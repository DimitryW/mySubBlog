...
import { getPostsByTag } from '@/api/postService'
...
const loadPosts = async (page = 1) => {
  ...
  try {
    const res = await getPostsByTag(tagName.value, page)
    posts.value = res.results
    currentPage.value = page
    totalPages.value = Math.ceil(res.count / 10) // 每頁 10 篇
  } catch (err) {
    console.error(err)
  } finally {
    loading.value = false
...
<template>
  <div class="page-wrapper">
    <div>
      <div class="header">
        <h1>Tag: <span class="category-name">{{ tagName }}</span></h1>
